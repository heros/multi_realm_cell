...
void MasteryScript::CalcAmount(AuraEffect const* aurEff, int32& amount, bool& canBeRecalculated)
{
    if (Unit* caster = GetCaster())
    {
        if (Player* player = caster->ToPlayer())
        {
            ...
            {
            //    amount = newAmount;
            //    sLog->outError(LOG_FILTER_TSCR, "Mastery spell %u eff %u: mastery %.2f, amount %u, bonus %d", aurEff->GetId(), aurEff->GetEffIndex(), mastery, baseAmount, newAmount);
            }
        }
    }
...
void MasteryScript::Register()
{
    for (uint8 i = EFFECT_0; i < MAX_SPELL_EFFECTS; i++)
    {
        ...
        {
            sLog->outError(LOG_FILTER_TSCR, "MasteryScript::Register mastery aura %u : %u", i, masteryAuras[i].auraType);
